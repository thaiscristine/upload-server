  ...
  it('should be able to export uploads', async () => {
    const uploadStub = vi
      ...
      .mockImplementationOnce(async () => {
        return {
          key: `${randomUUID()}-mocked-file.csv`,
          url: 'https://storage-example.com/mocked-key.csv',
        }
      })
    ...
    expect(unwrapEither(sut)).toEqual({
      reportUrl: 'https://storage-example.com/mocked-key.csv',
    })
    expect(csvAsArray).toEqual([
      ...
      [
        ...
        upload1.name,
        upload1.remoteUrl,
        expect.any(String), // We can use expect.any(String) here since the createdAt is a Date object that gets converted to a string in the CSV
      ],
